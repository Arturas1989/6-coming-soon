function renderSocials(selector, data) {
    ...
    if (!DOM) {
        return console.error(
            'ERROR: pagal pateikta selectoriu nepavyko rasti norimo elemento'
        );
    }
    let HTML = '';
    ...
    if (HTML === '') {
        return console.error(
            'ERROR: duomenu sarase nerasta nei vieno normalaus objekto'
        );
    }
...
function isValidInputData(selector, data) {
    ...
    if (data.length === 0) {
        return console.error(
            'ERROR: duomenu sarase turi buti bent vienas objektas'
        );
    }
}
